...
async function ensurePackages(packages) {
  if (process__default.env.CI || process__default.stdout.isTTY === false)
    return;
  const nonExistingPackages = packages.filter((i) => i && !localPkg.isPackageExists(i));
...
function createConfigForNuxt(options = {}, ...userConfigs) {
  ...
  if (resolved.features.formatters) {
    c.append(
      import('../chunks/formatters.cjs').then((m) => m.formatters(resolved.features.formatters, stylisticOptions))
    );
  }
  c.append(
    disables(resolved)
